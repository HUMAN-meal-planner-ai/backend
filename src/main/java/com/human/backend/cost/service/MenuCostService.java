 ...
 *
 * ■ 원가 계산 처리 흐름 (Calculation Flow):
 *   1. [파라미터 검증 & 보정] : 식수(mealCount >= 1), 목표단가(targetCost) 유효성 보정
 *   2. [식재료 구성 조회]      : CostRepository를 통해 메뉴에 등록된 식재료 목록 및 단가 조회
 *                              - 현재 원가: 최신 수집 단가(latestPrice)
 *                              - 미래 원가: 특정 일자 기준 예측 단가(predictedPrice)
 *   3. [1인분 식재료비 계산]  : 각 식재료별 중량(g) * 단가(원/g) 합산 (∑ LineCost)
 *   4. [총 예상 식재료비 산출] : 1인분 원가 * 총 식수 인원(mealCount)
 *   5. [목표 원가 초과 검사]   : 1인분 원가 > 목표단가 비교 ──> 초과 여부(isExceeded) 및 초과액(exceededAmount) 도출
 *   6. [응답 DTO 빌드]        : 계산된 총액 및 식재료별 세부 내역(IngredientDetail) 매핑 반환
 */
@Slf4j
@Service
